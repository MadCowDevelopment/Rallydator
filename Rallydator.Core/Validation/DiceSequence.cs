...
namespace Rallydator.Core.Validation
{
    internal class DiceSequence
    {
        ...
        {
            _damage = state.Damage.Clone();
            if(state.CurrentRoll.TireChanged) _damage.ChangeTire();
        ...
        public bool IsCompleteRoll => (IsInFinalGear || _currentRoll.FinalGear == 0) && (!IsTimeAttack || AllTimeAttackDiceUsed);
