    ...
    public static class CourseSectionFactory
    {
        ...
        private static IEnumerable<Space> CreateSection(string sectionDescription, Direction direction, string zone, int start)
        {
            ...
            for (var i = 0; i < spaceDescriptions.Length; i++)
            {
                ...
                previousSpaces = new List<Space>();
                foreach (var currentSpace in currentSpaces)
                {
                    previousSpaces.AddRange(FindEndSpaces(currentSpace));
                }
            }
        ...
        private static IEnumerable<Space> FindEndSpaces(Space currentSpace)
        {
            var result = new List<Space>();
            if (currentSpace.GetConnectedSpaces().Any())
            {
                foreach (var connectedSpace in currentSpace.GetConnectedSpaces())
                {
                    result.AddRange(FindEndSpaces(connectedSpace));
                }
            }
            else
            {
