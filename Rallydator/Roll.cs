...
namespace Rallydator
{
    public class Roll
    {
        ...
        {
            if (roll.Length + index < 4 || roll[index] != '(' || roll[index+1] != '-' || roll[index + 3] != ')') return 0;
            if (!int.TryParse(roll[index + 2].ToString(), out var spentSeconds)) return 0;
            if (spentSeconds != 1 && spentSeconds != 3 && spentSeconds != 6) ThrowError(roll, $"has invalid number of spent seconds '{spentSeconds}'.");
